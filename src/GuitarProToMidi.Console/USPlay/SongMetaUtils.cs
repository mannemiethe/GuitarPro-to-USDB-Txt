...
public static class SongMetaUtils
{
    ...
    {
        if (String.IsNullOrWhiteSpace(songMeta.Video))
        {
            return "";
        }
    ...
    public static Voice GetOrCreateVoice(SongMeta songMeta, string voiceName)
    {
        Voice matchingVoice = songMeta.GetVoices()
            .FirstOrDefault(voice => voice.Name == voiceName
                                     || (String.IsNullOrWhiteSpace(voiceName) && voice.Name == Voice.firstVoiceName)
                                     || (voiceName == Voice.firstVoiceName && String.IsNullOrWhiteSpace(voice.Name)));
